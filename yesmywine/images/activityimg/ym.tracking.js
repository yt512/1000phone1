...
try {
	var Tracking = {
		...
		isYmjUrl : function(url){
			url = url.trim();
			if (url.indexOf('http://')!=0 && url.indexOf('//')!=0){
				return true;
			}
			if (url.indexOf('yesmywine')>=0 || url.indexOf('yesmynet')>=0 || url.indexOf('ijiujiao')>=0){
				return true;
			}
			return false;
		},
		...
		processUrl : function(){
			...
			try{
				if (performance && performance.timing){
					this.tracking.domainLookupTime = (performance.timing.domainLookupEnd||new Date().getTime()) - (performance.timing.domainLookupStart||new Date().getTime());
					this.tracking.responseTime = (performance.timing.responseStart||new Date().getTime()) - (performance.timing.requestStart||new Date().getTime());
					this.tracking.domCompleteTime = (performance.timing.domComplete||new Date().getTime()) - (performance.timing.navigationStart||new Date().getTime());
					this.tracking.tcpConnectTime = (performance.timing.connectEnd||new Date().getTime()) - (performance.timing.connectStart||new Date().getTime());
				}
			}catch(e){}
		...
		post : function(){
			...
			for (var trackParam in this.tracking) {
				if (this.isNotBlank(this.tracking[trackParam])) {
					src += "&" + trackParam + "=" + encodeURIComponent(this.tracking[trackParam]);
				}
			}
			url = encodeURIComponent(url);
			refere = encodeURIComponent(refere);
		...
		getElementIndex : function(parentDts, $this,currEle,findTag) {
			...
			// 允许偏移量
			var start = parentDts.data("dtsStartIndex");
			if (start){
				index = (index) + parseInt(start);
			}
			return index;
		},
