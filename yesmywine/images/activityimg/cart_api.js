	...
	YM.page.cart = {
		...
		*/
		addGoodsToCartV2: function(optp) {
			...
			if (optp.yearGoodsId) urlParams.push('yearGoodsId=' + optp.yearGoodsId);
			if (optp.goodsYear) urlParams.push('goodsYear=' + optp.goodsYear);
		...
		addGoodsToCart: function(goodsId, goodsNum, callback, yearGoodsId, houseId, adSource) {
			...
			if (yGoodsId) urlParams.push('yearGoodsId=' + yGoodsId);
...
//			};
		...
		addGoodsToCartForCollege: function(classId, callback) {
			commonSpecialaddCartFn(['type=college','classId=' + classId], callback);
		},
		/****
         * 积分兑换
         */
